...
export function getStationCode(stationNames: StationNames, stationName: string) {
  ...
  return filteredStations.find(station => station.name === stationName || "pinyinCode" in station && station.pinyinCode === stationName || "pinyin" in station && station.pinyin === stationName || "pinyinInitials" in station && station.pinyinInitials === stationName)?.code
